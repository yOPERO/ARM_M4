...
//    Delay();
		//
		SW04 = GPIO_PORTF_DATA_R&0x11;
		oldPort = GPIO_PORTF_DATA_R&0x13;
		
		while(SW04 == 0x00 || SW04 == 0x01 || SW04 == 0x10){
			currentPort = GPIO_PORTF_DATA_R&0x13;
						
			if(oldPort != currentPort){
				getData();
				oldPort = currentPort;
			}
			GPIO_PORTF_DATA_R ^= 0x02;//TOOGLE
			if(oldPort != currentPort){
				getData();
				oldPort = currentPort;
			}			
